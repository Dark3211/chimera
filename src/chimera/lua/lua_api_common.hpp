...
namespace Chimera {
    ...
    static void lua_api_push_color(lua_State *state, const ColorRGB &color) noexcept {
        lua_createtable(state, 0, 3);
        lua_api_set_number(state, "red", color.red);
        lua_api_set_number(state, "green", color.green);
        lua_api_set_number(state, "blue", color.blue);
    }

    static const char *lua_api_network_role_name(NetworkedDatumRole role) noexcept {
        switch(role) {

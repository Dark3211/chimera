...
namespace Chimera {
    ...
    static bool lua_api_valid_gametype(Gametype type) noexcept {
        return type >= GAMETYPE_CTF && type <= GAMETYPE_RACE;
    }

    static const char *lua_api_gametype_type_name(Gametype type) noexcept {
        switch(type) {
            case GAMETYPE_CTF: return "ctf";
            case GAMETYPE_SLAYER: return "slayer";
            case GAMETYPE_ODDBALL: return "oddball";
            case GAMETYPE_KING: return "king";
            case GAMETYPE_RACE: return "race";
            default: return nullptr;
        }
    }

    static const char *lua_api_score_limit_unit(Gametype type) noexcept {
        switch(type) {
            case GAMETYPE_CTF: return "captures";
            case GAMETYPE_SLAYER: return "kills";
            case GAMETYPE_ODDBALL:
            case GAMETYPE_KING: return "seconds";
            case GAMETYPE_RACE: return "laps";
            default: return "unknown";
        }
    }

    static lua_Integer lua_api_score_limit_value(Gametype type, std::uint8_t raw_limit) noexcept {
        if(type == GAMETYPE_ODDBALL || type == GAMETYPE_KING) {
            return static_cast<lua_Integer>(raw_limit) * 60;
        }
        return static_cast<lua_Integer>(raw_limit);
    }
    ...
    static int lua_api_get_multiplayer_info(lua_State *state) noexcept {
        ...
        if(type == SERVER_NONE) {
            ...
            lua_pushnil(state);
            lua_setfield(state, -2, "gametype_name");
            lua_pushnil(state);
            lua_setfield(state, -2, "gametype_type_name");
            lua_pushnil(state);
            lua_setfield(state, -2, "team_game");
            lua_pushnil(state);
            lua_setfield(state, -2, "score_limit");
            lua_pushnil(state);
            lua_setfield(state, -2, "score_limit_unit");
            return 1;
        }

        const auto game_type = gametype();
        lua_api_set_integer(state, "gametype", game_type);
        lua_api_set_boolean(state, "team_game", is_team());

        if(lua_api_valid_gametype(game_type)) {
            const auto raw_score_limit = score_limit();
            lua_api_set_string(state, "gametype_type_name", lua_api_gametype_type_name(game_type));
            lua_api_set_integer(state, "score_limit", lua_api_score_limit_value(game_type, raw_score_limit));
            lua_api_set_string(state, "score_limit_unit", lua_api_score_limit_unit(game_type));
        }
        else {
            lua_pushnil(state);
            lua_setfield(state, -2, "gametype_type_name");

...
namespace Chimera {
    ...
    bool rasterizer_compile_shader(const char *source, const char *entry, const char *profile, D3D_SHADER_MACRO *defines, ID3DBlob **compiled_shader) {
        ...
        HRESULT result = D3DCompile(source, std::strlen(source), NULL, defines, D3D_COMPILE_STANDARD_FILE_INCLUDE, entry, profile, flags, 0, compiled_shader, &error_messages);
        if(FAILED(result)) {
    ...
    void set_up_rasterizer() noexcept {
        ...
        // This legacy fix patches DrawIndexedPrimitive and dynamic index-buffer
        // Lock vtables. Keep it on native D3D9, but do not stack it on D3D9On12.
